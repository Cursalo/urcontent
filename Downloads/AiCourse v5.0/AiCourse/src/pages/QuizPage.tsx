...
const QuizPage = () => {
    ...
    const handleStartQuiz = () => {
        ...
        toast({
            title: t('quiz.toast.started.title'),
            description: t('quiz.toast.started.description', { minutes: quizQuestions.length }),
        });
    };
    ...
    const handleNextQuestion = () => {
        if (currentQuestionIndex < quizQuestions.length - 1) {
            ...
            setTimerActive(false);
            toast({
                title: t('quiz.toast.completed.title'),
                description: t('quiz.toast.completed.description'),
            });
            setQuizResult();
        }
    };
                ...
                <div className="max-w-5xl mx-auto w-full flex items-center justify-between">
                    ...
                    {quizState === QuizState.InProgress && (
                        <div className="flex items-center gap-2 text-sm">
                            ...
                            <span>{formatTime(timeRemaining)} {t('quiz.timeRemaining')}</span>
                        </div>
                    )}
                </div>
            </header>
                        ...
                        <>
                            {quizState === QuizState.NotStarted && (
                                <Card className="text-center">
                                    <CardHeader>
                                        <CardTitle className="text-2xl capitalize">{t('quiz.title', { topic })}</CardTitle>
                                        <CardDescription>{t('quiz.subtitle', { topic })}</CardDescription>
                                    </CardHeader>
                                    <CardContent className="space-y-4">
                                        ...
                                        <div className="space-y-2 text-left">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span>{t('quiz.questionsToTest', { count: quizQuestions.length })}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <Clock className="h-5 w-5 text-muted-foreground" />
                                                <span>{t('quiz.minutesToComplete', { count: quizQuestions.length })}</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <Trophy className="h-5 w-5 text-muted-foreground" />
                                                <span>{t('quiz.instantFeedback')}</span>
                                            </div>
                                        </div>
                                    </CardContent>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-sm text-muted-foreground">
                                            <span>{t('quiz.questionOf', { current: currentQuestionIndex + 1, total: quizQuestions.length })}</span>
                                            <span>{formatTime(timeRemaining)} {t('quiz.remaining')}</span>
                                        </div>
                                    ...
                                    <Card>
                                        ...
                                        <CardFooter className="flex justify-between">
                                            ...
                                            >
                                                {currentQuestionIndex === quizQuestions.length - 1 ? t('quiz.finish') : t('quiz.next')}
                            ...
                            {quizState === QuizState.Completed && (
                                <div className="space-y-6">
                                    <Card className="text-center">
                                        <CardHeader>
                                            <CardTitle className="text-2xl">{t('quiz.title', { topic })} {passedQuiz ? t('quiz.passed') : t('quiz.failed')}</CardTitle>
                                            <CardDescription>
                                                {t('quiz.scored', { score: getScore(), total: quizQuestions.length })}
                                            </CardDescription>
                                        </CardHeader>
                                        <CardContent>
