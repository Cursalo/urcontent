...
const CoursePage = () => {
  ...
  const defaultMessage = `<p>${t('coursePage.teacher.defaultMessage', { topic: mainTopic })}</p>`;
  const defaultPrompt = `${t('coursePage.teacher.promptPrefix', { topic: mainTopic })} `;
  ...
  const handleSaveNote = async () => {
    ...
    if (response.data.success) {
      toast({
        title: t('coursePage.toast.noteSaved.title'),
        description: t('coursePage.toast.noteSaved.description'),
      });
    } else {
      toast({
        title: t('coursePage.toast.error.title'),
        description: t('coursePage.toast.error.description'),
      });
    }
  };
  ...
  async function getCombinedHtml(mainTopic, topics) {
    ...
    <div class="html2pdf__page-break" style="text-align: start; margin-top: 30px; margin-right: 16px; margin-left: 16px;">
        <h2 style="font-size: 24pt; font-weight: bold; margin: 0;">${t('coursePage.pdf.index')}</h2>
        <br>
        <hr>
        ${topicsHtml}
    </div>
  ...
  function certificateCheck() {
    if (isComplete) {
      ...
      toast({
        title: t('coursePage.completion'),
        description: t('coursePage.completeCourse'),
      });
    }
  }
        ...
        <div className="flex items-center gap-4">
          <Drawer>
            ...
            <DrawerContent className="max-h-[80vh]">
              <div className="p-4">
                <h2 className="text-xl font-bold mb-4">{t('coursePage.content')}</h2>
                <ScrollArea className="h-[60vh]">
                  <div className="pr-4">
                    {jsonData && renderTopicsAndSubtopics(jsonData[mainTopic.toLowerCase()])}
                    <p onClick={redirectExam} className='py-2 text-left px-3 hover:bg-accent/50 rounded-md cursor-pointer'>{pass === true ? <span className="mr-2 text-primary">✓</span> : <></>}{mainTopic} {t('coursePage.quiz')}</p>
                  </div>
                </ScrollArea>
              </div>
            </DrawerContent>
        ...
        <div className="flex items-center gap-2">
          ...
          <ToggleGroup type="single" className="hidden sm:flex">
            <Button variant="ghost" size="sm" asChild>
              <Link to='/dashboard'>
                <Home className="h-4 w-4 mr-1" /> {t('coursePage.home')}
              </Link>
            </Button>
            <Button onClick={certificateCheck} variant="ghost" size="sm" asChild>
              <span className='cursor-pointer'><Award className="h-4 w-4 mr-1" /> {t('coursePage.certificate')}</span>
            </Button>
            <Button onClick={htmlDownload} disabled={exporting} variant="ghost" size="sm" asChild>
              <span className='cursor-pointer'><Download className="h-4 w-4 mr-1" />{exporting ? t('coursePage.exporting') : t('coursePage.export')}</span>
            </Button>
            <ShareOnSocial
              textToShare={sessionStorage.getItem('mName') + " " + t('coursePage.shareText') + " " + mainTopic}
              link={websiteURL + '/shareable?id=' + courseId}
              linkTitle={sessionStorage.getItem('mName') + " " + t('coursePage.shareText') + " " + mainTopic}
              linkMetaDesc={sessionStorage.getItem('mName') + " " + t('coursePage.shareText') + " " + mainTopic}
              linkFavicon={appLogo}
              noReferer
            >
              <Button variant="ghost" size="sm" asChild>
                <span className='cursor-pointer'><Share className="h-4 w-4 mr-1" /> {t('coursePage.share')}</span>
              </Button>
            </ShareOnSocial>
          </ToggleGroup>
          <ThemeToggle />
      ...
      {isMobile ? (
          ...
          <SheetContent side="bottom" className="h-[90vh] sm:max-w-full p-0">
            <div className="flex flex-col h-full p-4">
              <div className="py-2 px-4 border-b border-border mb-2">
                <h2 className="text-lg font-semibold">{t('coursePage.assistant')}</h2>
              </div>
              ...
              <div className="flex items-center gap-2 p-4 border-t border-border">
                <Input
                  placeholder={t('coursePage.typeMessage')}
                  value={newMessage}
                  ...
                  className="flex-1"
                />
                <Button onClick={sendMessage}>{t('coursePage.send')}</Button>
              </div>
            </div>
          </SheetContent>
        </Sheet>
        ...
        <Dialog open={isChatOpen} onOpenChange={setIsChatOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogTitle>{t('coursePage.assistant')}</DialogTitle>
      ...
      {isMobile ? (
        <Sheet open={isNotesOpen} onOpenChange={setIsNotesOpen}>
          <SheetContent side="bottom" className="h-[90vh] sm:max-w-full p-0">
            <div className="flex flex-col h-full p-4">
              <div className="py-2 px-4 border-b border-border mb-2">
                <h2 className="text-lg font-semibold">{t('coursePage.notes')}</h2>
              </div>
              <ScrollArea className="flex-1 pr-4 mb-4">
                <div className="space-y-4 pt-2 px-4">
                  <MinimalTiptapEditor
              ...
              <div className="p-4 border-t border-border">
                <div className="flex justify-end">
                  <Button disabled={saving} onClick={handleSaveNote}>{saving ? t('coursePage.saving') : t('coursePage.saveNote')}</Button>
                </div>
              </div>
            </div>
          </SheetContent>
        ...
        <Dialog open={isNotesOpen} onOpenChange={setIsNotesOpen}>
          <DialogContent className="sm:max-w-lg">
            <DialogTitle>{t('coursePage.notes')}</DialogTitle>
            <div className="flex flex-col h-[60vh]">
              <ScrollArea className="flex-1 pr-4 mb-4">
                <div className="space-y-4 pt-2">
                  <MinimalTiptapEditor
