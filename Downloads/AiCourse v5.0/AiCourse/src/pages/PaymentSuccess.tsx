...
const PaymentSuccess = () => {
  ...
  const handleDownload = () => {
    ...
    toast({
      title: t('success.receiptDownloaded'),
      description: t('success.receiptDownloadedDesc'),
    });
  };
  ...
  async function getDetails() {
    ...
    if (sessionStorage.getItem('method') === 'stripe') {
      ...
      try {
        ...
        } else if (sessionStorage.getItem('method') === 'mercadopago') {
          // Get payment_id from URL params for MercadoPago
          const paymentId = searchParams.get('payment_id') || sessionStorage.getItem('mercadopago');
          const dataToSend = {
            paymentId: paymentId,
            uid: sessionStorage.getItem('uid'),
            plan: sessionStorage.getItem('plan')
          };
          const postURL = serverURL + '/api/mercadopagodetails';
          await axios.post(postURL, dataToSend).then(res => {
            sessionStorage.setItem('type', sessionStorage.getItem('plan'));
            sendEmail();
          });
  ...
  return (
    <>
      <SEO
        title={t('success.seo.title')}
        description={t('success.seo.description')}
        keywords={t('success.seo.keywords')}
      />
        ...
        <CardContent className="pt-6">
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-muted-foreground">{t('success.receipt')}</p>
            ...
            <div>
              <h3 className="font-medium mb-2">{t('success.planDetails')}</h3>
              <div className="flex justify-between items-center mb-1">
                <p>{planName}</p>
                <p className="font-bold">${cost}</p>
              </div>
              <p className="text-sm text-muted-foreground">
                {t('success.paymentMethod')}: {method}
              </p>
            </div>
            ...
            <div>
              <h3 className="font-medium mb-2">{t('success.billingDetails')}</h3>
