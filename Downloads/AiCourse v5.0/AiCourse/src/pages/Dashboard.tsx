    ...
    <>
      ...
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-gradient bg-gradient-to-r from-primary to-indigo-500">{t('title')}</h1>
          ...
          <>
            {courses.length > 0 ? (
                  ...
                  <Card key={course._id} className="overflow-hidden hover:shadow-lg transition-all duration-300 border-border/50 hover:border-primary/20 group">
                    <div className="aspect-video relative overflow-hidden">
                      ...
                      <div className="absolute top-2 right-2">
                        <Badge variant={course.status === 'Completed' ? 'destructive' : 'secondary'}>
                          {course.completed === true ? t('status.completed') : t('status.pending')}
                        </Badge>
                      </div>
                    ...
                    <CardContent className="pb-2">
                      <div className="mb-3">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">{courseProgress[course._id] || 0}% {t('progress.complete')}</p>
                      </div>
                      <div className="flex items-center justify-between text-sm text-muted-foreground">
                        <div className="flex items-center">
                          <BookOpen className="mr-1 h-4 w-4" />
                          {modules[course._id] || 0} {t('modules')}
                        </div>
                      </div>
                    </CardContent>
                    <CardFooter>
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h2 className="text-2xl font-bold mb-2">{t('emptyCourses.title')}</h2>
                <p className="text-muted-foreground max-w-md mb-6">
                  {t('emptyCourses.description')}
                </p>
                <Button size="lg" className="shadow-lg" asChild>
                  <Link to="/dashboard/generate-course">
                    <BookPlus className="mr-2 h-5 w-5" />
                    {t('emptyCourses.createFirst')}
                  </Link>
                </Button>
              </div>
            )}
