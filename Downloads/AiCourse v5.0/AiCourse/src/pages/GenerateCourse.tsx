...
const GenerateCourse = () => {
  ...
  const courseFormSchema = z.object({
    topic: z.string().min(3, { message: t('validation.topicMinLength') }),
...
type CourseFormValues = z.infer<typeof courseFormSchema>;
  ...
  const paidToad = () => {
    if (!paidMember) {
      toast({
        title: t('toast.goPremium.title'),
        description: t('toast.goPremium.description')
      });
    }
  };
  ...
  const addSubtopic = () => {
    if (subtopics.length < maxSubtopics) {
      ...
      toast({
        title: t('toast.upgradeToPremium.title'),
        description: t('toast.upgradeToPremium.description')
      });
    }
  };
  ...
  if (isSubmitted) {
    return (
      <>
        <SEO
          title={t('seo.titlePreview')}
          description={t('seo.descriptionPreview')}
          keywords={t('seo.keywordsPreview')}
        />
        <CoursePreview
          isLoading={isLoading}
    ...
    <>
      ...
      <div className="space-y-8 animate-fade-in max-w-3xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-gradient bg-gradient-to-r from-primary to-indigo-500 mb-4">{t('title')}</h1>
              ...
              <CardContent className="pt-6">
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <FormLabel>{t('form.subtopic.label')}</FormLabel>
                    <div className="flex gap-2">
                      <Input
                        placeholder={t('form.subtopic.placeholder')}
                        value={subtopicInput}
                  ...
                  <div>
                    <FormLabel>{t('form.topicsLimit.label')}</FormLabel>
                    <FormField
                      control={form.control}
                      name="topicsLimit"
                      render={({ field }) => (
                  ...
                  <div>
                    <FormLabel>{t('form.courseType.label')}</FormLabel>
                    <FormField
                      control={form.control}
                      name="courseType"
                      render={({ field }) => (
                          ...
                          <FormControl>
                            ...
                            >
                              <div className="flex items-center space-x-2 border p-3 rounded-md">
                                ...
                                <FormLabel htmlFor="ct1" className="mb-0">{t('form.courseType.textImage')}</FormLabel>
                              </div>
                              <div onClick={paidToad} className="flex items-center space-x-2 border p-3 rounded-md">
                                <RadioGroupItem disabled={!paidMember} value="Video & Text Course" id="ct2" />
                                <FormLabel htmlFor="ct2" className="mb-0">{t('form.courseType.videoText')}</FormLabel>
                              </div>
                            </RadioGroup>
                          </FormControl>
                        </FormItem>
