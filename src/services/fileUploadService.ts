// File Upload Service
// Comprehensive file handling with CDN integration and security
...
class FileUploadService {
  ...
  async uploadFile(
    ...
    try {
      // Rate limiting check
      const isAllowed = await rateLimitService.checkAPIRateLimit(
        options.userId,
        'upload'
      );
      
      if (!isAllowed) {
        throw new Error('Demasiados archivos subidos. Intenta nuevamente en una hora.');
      }
      ...
      const filename = this.generateFilename(file.name, fileId);
      ...
      if (createError) {
        throw new Error(`Error creating file record: ${createError.message}`);
      }
      ...
      const uploadResult = await this.uploadToStorage(file, filename, fileId, onProgress);
      
      if (!uploadResult.success) {
        await this.updateFileStatus(fileId, 'failed', uploadResult.error);
        throw new Error(uploadResult.error || 'Upload failed');
      }

      // Update file record with URL
      const updatedFile = await this.updateFileRecord(fileId, {
        url: uploadResult.url!,
        status: 'processing'
      });

      // Process file (compression, thumbnails, etc.)
      const processingResult = await this.processFile(updatedFile, options);
      
      if (processingResult.success && processingResult.file) {
        // Log successful upload
        await auditService.logUserAction(
          options.userId,
          (await authService.getCurrentUser())?.user?.email || 'unknown',
          'File uploaded',
          'file',
          fileId,
          {
  ...
  async uploadFiles(
    ...
    const results = await this.batchProcess(uploadPromises, 3); // Max 3 concurrent uploads
  ...
  async getFiles(
    ...
    try {
      ...
      if (filters.search_term) {
        query = query.or(
          `filename.ilike.%${filters.search_term}%,original_filename.ilike.%${filters.search_term}%`
        );
      }
  ...
  async getFileStats(userId?: string): Promise<{
    ...
    files_by_category: Record<FileCategory, number>;
    storage_used: string;
    storage_limit: string;
  }> {
    try {
      const filters: FileFilters = { status: 'ready' };
      if (userId) {
        filters.user_id = userId;
      }

      const files = await this.getFiles(filters, 10000);
      
      const stats = {
        total_files: files.length,
        total_size: files.reduce((sum, file) => sum + file.file_size, 0),
        files_by_type: {} as Record<FileType, number>,
        files_by_category: {} as Record<FileCategory, number>,
        storage_used: '',
        storage_limit: '10GB' // Mock limit
      };

      // Count by type and category
      files.forEach(file => {
        stats.files_by_type[file.file_type] = (stats.files_by_type[file.file_type] || 0) + 1;
        stats.files_by_category[file.category] = (stats.files_by_category[file.category] || 0) + 1;
      });

      stats.storage_used = this.formatFileSize(stats.total_size);
  ...
  private validateFile(file: File, options: FileUploadOptions): { isValid: boolean; error?: string } {
    // Check file size
    const maxSize = options.maxSize || this.MAX_FILE_SIZE.other;
    ...
    // Check file type
    const fileType = this.getFileType(file.type);
    const allowedTypes = options.allowedTypes || this.ALLOWED_TYPES[fileType] || [];
  ...
  private async processFile(
    ...
    try {
      ...
      if ((file.file_type === 'image' || file.file_type === 'video') && options.generateThumbnail) {
        thumbnailUrl = await this.generateThumbnail(file);
      }

      // Compress images if requested
      if (file.file_type === 'image' && options.compress) {
        compressedUrl = await this.compressImage(file);
      }

      // Update file record with processing results
      const updatedFile = await this.updateFileRecord(file.id, {
        status: 'ready',
        thumbnail_url: thumbnailUrl,
        cdn_url: compressedUrl || file.url,
      ...
      await this.updateFileStatus(file.id, 'failed', error.message);
      return {
        success: false,
        error: error.message
      };
  ...
  private async generateThumbnail(file: UploadedFile): Promise<string | undefined> {
    try {
      ...
      return file.url; // In production, return actual thumbnail URL
    } catch (error) {
  ...
  private async compressImage(file: UploadedFile): Promise<string | undefined> {
    try {
      // In production, this would use an image compression service
      console.log(`🗜️ Compressing image ${file.filename}`);
      
      // Mock compression
      return file.url; // In production, return compressed image URL
    } catch (error) {
