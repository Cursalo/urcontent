...
class BusinessService {
  ...
  async createBusinessProfile(
    userId: string,
    profileData: Omit<TablesInsert<'business_profiles'>, 'user_id'>
  ...
  async updateBusinessProfile(
    id: string,
    updates: TablesUpdate<'business_profiles'>
  ): Promise<BusinessProfile> {
    const { data, error } = await supabase
      .from('business_profiles')
      .update({
  ...
  async getBusinessAnalytics(userId: string): Promise<Tables<'user_analytics'>[]> {
    const { data, error } = await supabase
      .from('user_analytics')
      .select('*')
      .eq('user_id', userId)
  ...
  // Search businesses
  async searchBusinesses(query: string, limit: number = 20): Promise<BusinessProfile[]> {
    const { data, error } = await supabase
      .from('business_profiles')
      .select(`
        *,
      ...
      `)
      .or(
        `company_name.ilike.%${query}%,description.ilike.%${query}%,industry.ilike.%${query}%`
      )
      .limit(limit)
      .order('is_verified_business', { ascending: false });
  ...
  async getBusinessesByIndustry(industry: string, limit: number = 20): Promise<BusinessProfile[]> {
    const { data, error } = await supabase
      .from('business_profiles')
      .select(`
        *,
  ...
  async getVerifiedBusinesses(limit: number = 20): Promise<BusinessProfile[]> {
    const { data, error } = await supabase
      .from('business_profiles')
      .select(`
        *,
  ...
  async updateVerificationStatus(id: string, isVerified: boolean): Promise<BusinessProfile> {
    return this.updateBusinessProfile(id, {
      is_verified_business: isVerified,
    });
  }

  // Add venue to business profile
  async addVenue(
  ...
  // Get business collaboration history
  async getCollaborationHistory(businessId: string): Promise<any[]> {
    const { data, error } = await supabase
      .from('collaborations')
      .select(`
        *,
  ...
  async getBusinessDashboardStats(businessId: string): Promise<{
    ...
    if (collabError) {
      console.error('Error fetching business dashboard stats:', collabError);
      throw collabError;
    }

    const totalCampaigns = collaborations?.length || 0;
    const activeCampaigns = collaborations?.filter(c => 
      ['accepted', 'in_progress'].includes(c.status || '')
    ).length || 0;
    const completedCampaigns = collaborations?.filter(c => 
      c.status === 'completed'
    ).length || 0;
    const totalSpent = collaborations?.reduce((sum, c) => 
      sum + (c.compensation_amount || 0), 0
    ) || 0;

    // TODO: Calculate avgRating from reviews
    const avgRating = 0;

    // TODO: Calculate totalReach from analytics
    const totalReach = 0;
  ...
  async getRecentActivity(businessId: string, limit: number = 10): Promise<any[]> {
    const { data, error } = await supabase
      .from('collaborations')
      .select(`
        *,
