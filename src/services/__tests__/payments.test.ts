...
describe('PaymentsService', () => {
  ...
  describe('getPayment', () => {
    ...
    it('returns null when payment not found', async () => {
      ...
      const payment = await paymentsService.getPayment('non-existent-id')
  ...
  describe('getPaymentByMPId', () => {
    ...
    it('returns null when payment not found', async () => {
      ...
      const payment = await paymentsService.getPaymentByMPId('non-existent-mp-id')
  ...
  describe('createPayment', () => {
    it('creates payment with correct data', async () => {
      ...
      expect((mockSupabase.from as any)().insert).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          description: 'Test Payment',
          payment_method: 'MercadoPago',
          transaction_id: expect.stringMatching(/^TXN-\d+-.{9}$/),
        })
      )
      expect(payment).toEqual(mockPayment)
    })
    ...
    it('throws error when creation fails', async () => {
      ...
      await expect(paymentsService.createPayment(paymentData)).rejects.toThrow('Database error')
    })
  })
  ...
  describe('getPaymentStats', () => {
    it('calculates payment statistics correctly', async () => {
      const mockPayments = [
        createMockPayment({ status: 'approved', amount: 10000, type: 'membership' }),
        createMockPayment({ status: 'approved', amount: 20000, type: 'collaboration' }),
      ...
      expect(stats.total_amount_approved).toBe(30000)
      expect(stats.total_amount_pending).toBe(15000)
      expect(stats.total_transactions).toBe(4)
      expect(stats.avg_transaction_amount).toBe(12500)
      expect(stats.by_type.membership.count).toBe(2)
      expect(stats.by_type.membership.amount).toBe(25000)
      expect(stats.by_type.collaboration.count).toBe(1)
      expect(stats.by_type.collaboration.amount).toBe(20000)
    })
    ...
    it('handles empty payment list', async () => {
      ...
      expect(stats.avg_transaction_amount).toBe(0)
      expect(stats.total_amount_approved).toBe(0)
    })
  })
    ...
    it('creates basic membership payment with correct amount', async () => {
      ...
      expect((mockSupabase.from as any)().insert).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          description: 'URContent Basic - Plan Mensual',
  ...
  describe('processMercadoPageWebhook', () => {
    it('processes approved payment webhook', async () => {
      ...
      const result = await paymentsService.processMercadoPageWebhook('mp-123', 'approved')
    ...
    it('processes rejected payment webhook', async () => {
      ...
      const result = await paymentsService.processMercadoPageWebhook('mp-123', 'rejected', 'cc_rejected_insufficient_amount')
    ...
    it('handles unknown payment status', async () => {
      ...
      expect(result).toEqual(mockPayment) // Returns original payment without update
    })
  })

  describe('exportPayments', () => {
    it('exports payments to CSV format', async () => {
      const mockPayments = [
        createMockPayment({
          id: 'payment-1',
          user_id: 'user-1',
      ...
      expect(csvContent).toContain('ID,Fecha,Usuario,Tipo,Estado,Monto')
      expect(csvContent).toContain('"payment-1"')
      expect(csvContent).toContain('"payment-2"')
      expect(csvContent).toContain('"membership"')
      expect(csvContent).toContain('"collaboration"')
  ...
  describe('refundPayment', () => {
    it('refunds approved payment successfully', async () => {
      ...
      const result = await paymentsService.refundPayment('payment-123', 'Customer request')
