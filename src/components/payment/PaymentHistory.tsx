...
export const PaymentHistory: React.FC<PaymentHistoryProps> = ({ userId: propUserId, userType }) => {
  const { user } = useAuth();
  ...
  const { data: payments = [], isLoading, error, refetch } = useQuery({
    queryKey: ['payments', currentUserId, filters],
    queryFn: () => {
      ...
      if (searchTerm) {
        queryFilters.search_term = searchTerm;
      }
      if (statusFilter !== 'all') {
        queryFilters.status = statusFilter as any;
      }
      if (typeFilter !== 'all') {
        queryFilters.type = typeFilter as any;
      }
      
      return paymentsService.getPayments(queryFilters, 100);
    },
  ...
  const exportMutation = useMutation({
    mutationFn: async () => {
      ...
      return paymentsService.exportPayments(queryFilters);
    },
    onSuccess: (csvData) => {
    ...
    onError: (error: any) => {
      toast.error(error?.message || 'Error al exportar pagos');
    }
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 border-green-200">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <p className="text-sm font-medium text-green-700">Pagos Aprobados</p>
              <p className="text-2xl font-bold text-green-900">
                {formatCurrency(amounts.approved)}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-900">
                {formatCurrency(amounts.pending)}
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <Select value={typeFilter} onValueChange={handleTypeFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="campaign_deposit">Depósito</SelectItem>
            </SelectContent>
          </Select>
                        ...
                        <div>
                          ...
                          {payment.metadata && (
                            <div className="flex items-center space-x-4 mt-2">
                              {payment.metadata.creator_name && (
                                <span className="text-xs text-gray-500">
                                  Creador: {payment.metadata.creator_name}
                                </span>
                              )}
                              ...
                              {payment.metadata.membership_tier && (
                                <span className="text-xs text-gray-500">
                                  Plan: {payment.metadata.membership_tier}
                                </span>
                              )}
                            </div>
                          )}
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-gray-600">{getTypeLabel(payment.type)}</p>
                                </div>
                              </div>
                              {getStatusBadge(payment.status)}
                            </div>
