...
export const PaymentButton: React.FC<PaymentButtonProps> = ({
  ...
  const getButtonStyles = () => {
    ...
    const variantStyles = {
      ...
      gradient: 'bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 text-white'
    };
      ...
      {showInstallments && hasInstallments && !loading && (
        <Card className="p-4 bg-blue-50 border-blue-200">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="font-medium text-blue-900">Opciones de Pago</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {installmentOptions.map((installments) => (
              <Badge key={installments} variant="outline" className="text-blue-700 border-blue-300">
                {installments === 1 
                  ? 'Pago único' 
