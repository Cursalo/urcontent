...
const PanelPrincipalBusiness: React.FC = () => {
  const { metricas, rendimientoCampanas, distribucionPlataformas, campanasActivas, creatorsDestacados, calendarioCampanas } = businessMockData;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metricasConfig.map((metrica, index) => (
          <Card key={index} className="bg-white border border-gray-100 rounded-2xl hover:border-gray-200 transition-all duration-300 hover:shadow-lg">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className={`w-12 h-12 bg-gradient-to-br ${colores[metrica.color as keyof typeof colores]} rounded-2xl flex items-center justify-center`}>
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className={`text-sm font-medium ${metrica.cambio.esPositivo ? 'text-green-500' : 'text-red-500'}`}>
              ...
              {distribucionPlataformas.map((plataforma, index) => (
                <div key={index} className="space-y-2">
                  ...
                  <Progress 
                    ...
                    className="h-2"
                    style={{
                      '--progress-background': plataforma.color
        ...
        <Card className="bg-white border border-gray-100 rounded-2xl">
          ...
          <CardContent className="space-y-4">
            {campanasActivas.slice(0, 3).map((campana) => (
              <div key={campana.id} className="space-y-3 p-4 bg-gray-50 rounded-xl">
                <div className="flex items-start justify-between">
                  ...
                  <Badge
                    variant={
                      campana.estado === 'en_progreso' ? 'default' :
                      campana.estado === 'revision' ? 'secondary' :
                      'outline'
                    }
                  >
        ...
        <Card className="bg-white border border-gray-100 rounded-2xl">
          ...
          <CardContent className="space-y-4">
            {creatorsDestacados.slice(0, 3).map((creator) => (
              <div key={creator.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                <div className="flex items-center space-x-3">
      ...
      <Card className="bg-gradient-to-br from-black to-gray-900 text-white rounded-2xl">
        <CardHeader>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {calendarioCampanas.map((evento) => (
              <div key={evento.id} className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    evento.tipo === 'inicio' ? 'bg-green-500/20' :
                    evento.tipo === 'revision' ? 'bg-yellow-500/20' :
                    'bg-blue-500/20'
                  }`}>
                    {evento.tipo === 'inicio' ? <Clock className="w-5 h-5 text-green-400" /> :
                     evento.tipo === 'revision' ? <AlertCircle className="w-5 h-5 text-yellow-400" /> :
